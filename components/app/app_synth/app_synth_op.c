...
esp_err_t app_synth_op_set (app_synth_op_t *op, app_synth_env_t *env, app_synth_wave_t wave, float freq, uint8_t coarse, float level) {
    app_synth_osc_set(&op->osc, wave, coarse ? freq * coarse : freq / 2);  // coarse = 0表示1/2
    op->env = env;
    op->phase = 0;
    op->phase_inc = op->osc.phase_inc;
    op->env_level = 0;
    op->env_state = env_state_attack;
    op->level = level;
...
esp_err_t app_synth_op_modulate (app_synth_op_t *carrier, const app_synth_op_t *modulator) {
    ...
    int32_t fm = (mod << 15) * modulator->env_level * modulator->level;
